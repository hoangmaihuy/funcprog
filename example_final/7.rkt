...
(define (make-machine register-names ops controller-text breakpoint)
  (let ((machine (make-new-machine)))
    ...
    ((machine 'install-breakpoint) breakpoint)
    machine))
...
(define (make-new-machine) 
  ...
  (let ((pc (make-register 'pc)) 
    ...
    (let ((the-ops
      ...
      (define (execute)
        (let ((insts (get-contents pc)))  
          ...
          (if (null? insts)
              (void)
              (begin
                (let ([bp-regs (assoc insts the-breakpoints)])
      ...
      (define (make-breakpoint pc regs)
        (cons pc regs)
      )

      (define (install-breakpoint breakpoint)
        (if (null? breakpoint)
          (void)
          (let ([label (car breakpoint)]
            ...
            (set! 
              the-breakpoints
              (cons
                (make-breakpoint (get-contents pc) regs)
                the-breakpoints
              )
            ) 
          )
...
(define (make-machine-from-text machine-text)
  ...
  (if (null? (cddddr machine-text))
    (void)
    (set! breakpoint (car (cddddr machine-text)))
  )
  (make-machine reg-table op-table controller breakpoint)
)
