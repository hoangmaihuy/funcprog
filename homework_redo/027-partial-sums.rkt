...
;--- starts here
	(define (accumulate total stream)
		(let ([new-total (op total (stream-car stream))])
			(cons-stream
				new-total
				(accumulate new-total (stream-cdr stream))
			)
		)
	)
	(cons-stream (stream-car s) (accumulate (stream-car s) (stream-cdr s)))
 )
;--- ends here
env)
