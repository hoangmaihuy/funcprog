...
(define (adder-list a b s)
	(if (null? s)
		'()
		(cons (adder (car a) (car b) (car s)) (adder-list (cdr a) (cdr b) (cdr s)))
	)
)

(define (get-value-list l)
	(if (null? l)
		'()
		(cons (get-value (car l)) (get-value-list (cdr l)))
	)
)

(define (set-value-list! l ref val informant)
	(if (= ref 0)
		(set-value! (car l) val informant)
		(set-value-list! (cdr l) (- ref 1) val informant)
	)
)
