...
(define assemble-error '*unassigned*)


(define (assemble controller-text machine)
  (set! assemble-error '*unassigned*)
         ...
         (let ((next-inst (car text)))
           (if (symbol? next-inst)
                 (if (assoc next-inst labels)
                     (set! assemble-error "label error in machine")
                     (receive insts
...
(define (make-swap inst machine pc)
  (let ([reg1-name (cadr inst)]
        [reg2-name (caddr inst)])
    (lambda ()
      (let ([reg1-val (get-register-contents machine reg1-name)]
          [reg2-val (get-register-contents machine reg2-name)])
        (set-register-contents! machine reg1-name reg2-val)
        (set-register-contents! machine reg2-name reg1-val)
        (advance-pc pc)
      )
    )
  )
            ...
            (if (eq? m-txt eof)
                ...
                (if (eq? (car m-txt) 'make-machine)
                    (begin 
                           ...
                           (if (not (eq? assemble-error '*unassigned*))
                             (begin
                               (displayln assemble-error)
                               (set! machine '*unassigned*)
                               (inner-loop)
                             )
                             (begin
