        ...
        ((compound-procedure? procedure)
         ...
         (eval-sequence
          ...
          (extend-environment  
           ...
           (list->mlist (list-of-delayed-args (procedure-parameter-types procedure) arguments env)) 
...
(define (make-arg type arg env)
	(cond 
		[(eq? type 'strict) (eval arg env)]
		[(eq? type 'lazy) (delay-it arg env)]
		[(eq? type 'lazy-memo) (delay-memo-it arg env)]
		[else (error "Unknown arg type: MAKE-ARG" type arg)]
	)
)

(define (list-of-delayed-args arg-types exps env) 
  (if (no-operands? exps)
      '()
			(cons 
				(make-arg (car arg-types) (first-operand exps) env) 
				(list-of-delayed-args (cdr arg-types) (rest-operands exps) env)
			)
	)
)
