...
(define (analyze exp)   
   (cond 
				 [(try-again? exp) (analyze-try-again exp)]
	 			 ((self-evaluating? exp) 
	...
	(let ([bproc (analyze (if-fail-body exp))]
		...
		(lambda (env succeed fail)
			(bproc
				env
				; bproc succeed
				(lambda (bval alternative)
					(set! is-success #t)
					(succeed bval alternative)
				)
				; bproc fail
				(lambda ()
					(if (not is-success)
						(cproc
							env
							(lambda (cval alternative)
								(succeed cval alternative)
							)
							fail
						)
						(fail)
...
(define (try-again? exp)
	(tagged-list? exp 'try-again)
)

(define (try-again-times exp)
  (cadr exp)
)

(define (analyze-try-again exp)
	(lambda (env succeed fail)
		(succeed 'try-again fail)
	)
)
...
(define next-try glb-fail)

(define glb-succeed 
  (lambda (val next)
		(cond 
			[(eq? val (void)) (void)]
			[(eq? val 'try-again) (next-try)]
			[else (begin (set! next-try next) (display val) (newline))]
		)
	)
)
...
(let ((input (read))) 
	(if (eq? input eof)
			(void)
			(begin (if (try-again? input) (void) (set! next-try glb-fail))
							(ambeval input glb-env glb-succeed glb-fail)
							(my-driver-loop)))))
(my-driver-loop)
