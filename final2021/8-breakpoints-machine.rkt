...
(define (make-machine register-names ops controller-text breakpoints)
  (let ((machine (make-new-machine)))
    ...
    ((machine 'install-breakpoints) breakpoints)
    machine))
...
(define (make-new-machine) 
  ...
  (let ((pc (make-register 'pc)) 
    ...
    (let ((the-ops
      ...
      (define (execute)
        (let ((insts (get-contents pc)))  
          ...
          (if (null? insts)
              (void)
              (begin
                (let ([bp-info (assoc insts the-breakpoints)])
                  ; (display "bp-regs: ") (display bp-regs) (newline)
                  (if bp-info
                    (begin
                      (display "at breakpoint ") (display (cadr bp-info)) (display ":")
                      (for-each 
                        (lambda (reg-name)
                          (display (get-contents (lookup-register reg-name)))
                          (display " ")
                        )
                        (cddr bp-info)
                      )
      ...
      (define (make-breakpoint pc regs idx)
        (cons pc (cons idx regs))
      )

      (define (install-breakpoint breakpoint idx)
        (if (null? breakpoint)
          (void)
          (let ([label (car breakpoint)]
                [ref (cadr breakpoint)]
                [regs (cddr breakpoint)])
            (set-contents! pc (lookup-label the-labels label))
            (for ([i (in-range ref)])
              (advance-pc pc)
            )
            (set! 
              the-breakpoints
              (cons
                (make-breakpoint (get-contents pc) regs idx)
                the-breakpoints
              )
            ) 
          )
      ...
      (define (install-breakpoints breakpoints)
        (define (install-loop bps idx)
          (if (null? bps)
            (void)
            (begin
              (install-breakpoint (car bps) idx)
              (install-loop (cdr bps) (+ idx 1))
            )
          )
        )
        (install-loop breakpoints 0)
      )
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'install-instruction-sequence) 
                ...
                (lambda (seq) 
                  (set! the-instruction-sequence (car seq))
                  (set! the-labels (cadr seq))
                )
              )
              ((eq? message 'install-breakpoints) install-breakpoints)
...
(define (make-machine-from-text machine-text)
  ...
  (define breakpoints '())
  (if (null? (cddddr machine-text))
    (void)
    (set! breakpoints (cddddr machine-text))
  )
