...
;implemented switch, while, for, break
...
(define (switch-cases exp)
	(cddr exp)
)

(define (switch-conditions exp)
	(map car (switch-cases exp))
)

(define (switch-actions exp)
	(map cdr (switch-cases exp))
)

(define (analyze-switch exp)
	(let ([vproc (analyze (switch-value exp))]
				[conditions (switch-conditions exp)]
				[actions (switch-actions exp)])
		(lambda (env)
			(let ([value (vproc env)])
				(define (switch-loop conds acts)
					(cond
						[(null? conds) (error "No default case: SWITCH")]
						[(eq? (car conds) 'default) ((analyze-sequence (car acts)) env)]
						[else
							(let ([cproc (analyze (car conds))]
										[aproc (analyze-sequence (car acts))])
								(if (equal? value (cproc env))
									(aproc env)
									(switch-loop (cdr conds) (cdr acts))
								)
							)
						]
					)
				)
				(switch-loop conditions actions)
			)
		)
	)
)
...
(define (analyze-for exp)
	(let ([iproc (analyze (for-init exp))]
		...
		(lambda (env)
			(define (for-loop)
				(define (body-loop bprocs)
					(if (null? bprocs)
						(begin (sproc env) (for-loop))
						(let ([bval ((car bprocs) env)])
							(if (eq? bval 'break)
								(void)
								(body-loop (cdr bprocs))
							)
						)
					)
				)
