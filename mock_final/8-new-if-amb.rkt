...
(define (new-if-predicate exp)
	(cadr exp)
)

(define (new-if-success exp)
	(caddr exp)
)

(define (new-if-fail exp)
	(cadddr exp)
)

(define (analyze-new-if exp)
	(let ([pproc (analyze (new-if-predicate exp))]
				[sproc (analyze (new-if-success exp))]
				[fproc (analyze (new-if-fail exp))])
