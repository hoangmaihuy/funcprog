...
;---code starts here
  (define (raise-rational x)
    (make-real (/ (car (contents x)) (cdr (contents x))))
  )
...
(define (super-type? t1 t2)
  (define type-list '(integer rational real complex))
  ...
  (> (index-of type-list t1) (index-of type-list t2))
)
  ...
  (let ((proc (get op type-tags)))
    (if proc
      ...
      (let ((t1 (car type-tags))
            ...
            (x2 (cadr args)))
        (if (super-type? t1 t2)
          (apply-generic op x1 (raise-type x2))
          (apply-generic op (raise-type x1) x2)
        )
      )
    )
  ))
...
(define (display-obj obj)
  (define (display-integer x) (printf "(integer ~a)\n" x))
  (define (display-rational x) (printf "(rational ~a ~a)\n" (car x) (cdr x)))
  (define (display-real x) (printf "(real ~a)\n" x))
  (define (display-complex x) (printf "(complex ~a ~a)\n" (real-part x) (imag-part x)))
  (let ((obj-tag (type-tag obj))
        (obj-content (contents obj)))
    (cond ((eq? obj-tag 'integer) (display-integer obj-content))
          ((eq? obj-tag 'rational) (display-rational obj-content))
          ((eq? obj-tag 'real) (display-real obj-content))
          ((eq? obj-tag 'complex) (display-complex obj-content))
          (else (error "No display function for" obj))
    )
  )
)
;----code ends here
