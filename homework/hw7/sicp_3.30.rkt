...
;--- starts here
  (define (loop-add wire-lstA wire-lstB wire-lstS wireC)
    (if (null? wire-lstS)
      (void)
      (let ([a (car wire-lstA)]
            [b (car wire-lstB)]
            [c-in wireC]
            [s (car wire-lstS)]
            [c-out (if (null? (cdr wire-lstS)) wireCN (make-wire))])
        (full-adder a b c-in s c-out)
        (loop-add (cdr wire-lstA) (cdr wire-lstB) (cdr wire-lstS) c-out)
      )
    )
  )
  
  (loop-add wire-lstA wire-lstB wire-lstS (make-wire))
)
;---ends here
